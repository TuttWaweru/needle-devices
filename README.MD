...
<img src="/media/needle_icon_large.png"/>

Needle is an advertising platform for the medical industry
...
This demonstrates sharing not only the data and domain layers of the app but also the
application state:
...
There are two types of data sources. The network service is for getting api updates, while
local storage is for caching the api responses, which makes it possible to use the application
...
organized in the shared module of the `needle.devices.com.core` package.
...
the `needle.devices.com.app` package.
...
For subscribing to state updates, the simple wrapper is used. 
This wrapper allows you to provide a callback that will be called when each new value (the
